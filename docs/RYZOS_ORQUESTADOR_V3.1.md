...
* **Pecuario (v1+v2+v3 aplicadas y confirmadas en vivo, última 2026-09-11):** v1 (`PECUARIO_CONFIGURACION`/`JAULAS`/`PARTOS`/`LOTES`/`PESAJES`/`MORTALIDAD`/`VENTAS`), v2 (`PECUARIO_GALPONES`/`CONTROL_SANITARIO`/`LIMPIEZA_GALPON`/`INSUMOS`/`INSUMOS_MOVIMIENTOS` — cierra el gap de `PECUARIO_JAULAS.galpon_id` sin FK que v1 había dejado abierto), v3 (`PECUARIO_REPRODUCTORES`/`HISTORIAL_MACHOS`/`TRATAMIENTOS`, identificación individual de reproductoras/reproductores, overlay opcional sobre el manejo poblacional). Ver `docs/schema_live_pecuario.md` para el schema completo columna por columna y `docs/ESTADO_PROYECTO.md` para el detalle de cada hito. La organización tenant (`GRANJA-VALENCIA`) fue dada de alta el 2026-09-10 (`ORGANIZACION_PRODUCTOS` sin fila para ella a propósito — el catálogo `PRODUCTOS` solo cubre `CAFE`/`CACAO` todavía).

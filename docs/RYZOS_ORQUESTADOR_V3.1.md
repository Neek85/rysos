...
* **Pecuario (MVP creado 2026-09-10, migración aplicada en Supabase):** `PECUARIO_CONFIGURACION`, `PECUARIO_JAULAS`, `PECUARIO_PARTOS`, `PECUARIO_LOTES`, `PECUARIO_PESAJES`, `PECUARIO_MORTALIDAD`, `PECUARIO_VENTAS` (creadas por `supabase/migrations/20260910160000_pecuario_cuyes_core.sql`, verificado en vivo que las 7 tablas existen en la instancia real). **Pendiente:** `PECUARIO_GALPONES` todavía no existe (la columna `galpon_id` en `PECUARIO_JAULAS` queda sin llave foránea hasta que se cree). La organización tenant (`GRANJA-VALENCIA`) **fue dada de alta el 2026-09-10** (`INSERT` del runbook `specs/alta_organizacion_real.md`, aplicado manualmente en Supabase Studio y confirmado en vivo contra `ORGANIZACIONES`). Antes de esa fecha, ni la organización ni ninguna tabla `PECUARIO_*` existían en la base real, pese a que versiones anteriores de este documento las daban por activas y operando. `ORGANIZACION_PRODUCTOS` no tiene fila para `GRANJA-VALENCIA` a propósito (decisión confirmada 2026-09-10: el catálogo `PRODUCTOS` solo cubre `CAFE`/`CACAO`, sin producto pecuario todavía — ver `docs/ESTADO_PROYECTO.md`).

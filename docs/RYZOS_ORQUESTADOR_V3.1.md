...
* **Pecuario (MVP creado 2026-09-10, migración aplicada en Supabase):** `PECUARIO_CONFIGURACION`, `PECUARIO_JAULAS`, `PECUARIO_PARTOS`, `PECUARIO_LOTES`, `PECUARIO_PESAJES`, `PECUARIO_MORTALIDAD`, `PECUARIO_VENTAS` (creadas por `supabase/migrations/20260910160000_pecuario_cuyes_core.sql`, verificado en vivo que las 7 tablas existen en la instancia real). **Pendiente:** `PECUARIO_GALPONES` todavía no existe (la columna `galpon_id` en `PECUARIO_JAULAS` queda sin llave foránea hasta que se cree). La organización tenant (`GRANJA-VALENCIA`) **todavía no fue dada de alta** — el `INSERT` del runbook (`specs/alta_organizacion_real.md`) está listo y verificado contra el esquema real, pendiente de aplicación manual en Supabase Studio (ver `docs/ESTADO_PROYECTO.md` para el estado exacto). Antes de esa alta, ni la organización ni ninguna tabla `PECUARIO_*` existían en la base real, pese a que versiones anteriores de este documento las daban por activas y operando.

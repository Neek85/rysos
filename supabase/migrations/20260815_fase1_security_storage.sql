...
-- 2. FUNCIÓN HELPER EXTRACTORA DE JWT
...
-- 4. POLÍTICAS UNIFICADAS MULTI-TENANT
...
-- 5. TRIGGER DE INYECCIÓN AUTOMÁTICA DE ORG_ID
...
-- 6. VISTA CONTROL DE CALIDAD APROBADOS
...
-- 7. SUPABASE STORAGE Y POLÍTICAS DE RLS
...
CREATE POLICY "ryzos_storage_select" ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'evidencias_eudr' AND (storage.foldername(name))[1] = public.get_my_org_id());

DROP POLICY IF EXISTS "ryzos_storage_insert" ON storage.objects;
CREATE POLICY "ryzos_storage_insert" ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'evidencias_eudr' AND (storage.foldername(name))[1] = public.get_my_org_id());

DROP POLICY IF EXISTS "ryzos_storage_delete" ON storage.objects;
CREATE POLICY "ryzos_storage_delete" ON storage.objects FOR DELETE TO authenticated
USING (bucket_id = 'evidencias_eudr' AND (storage.foldername(name))[1] = public.get_my_org_id());

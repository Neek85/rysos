-- MIGRACIÓN IDEMPOTENTE: corregir mismatch de tipos uuid/text en
-- fn_guardar_inspeccion_completa().
--
-- CONTEXTO: encontrado durante la verificación de Fase C Paso 1 (login
-- real por organización y rol, specs/login_real_organizacion_rol.md) --
-- no relacionado al login en sí. INSPECCIONES."ID_Inspeccion" es de tipo
-- text (no uuid), pero fn_guardar_inspeccion_completa() (creada en
-- 20260818_inspecciones_atomic_save.sql) declara su parámetro p_id y su
-- variable local v_id como uuid. Cualquier intento de creación de
-- inspección nueva falla con:
--   42883 operator does not exist: text = uuid
-- al comparar/insertar v_id (uuid) contra columnas "ID_Inspeccion" (text)
-- en INSPECCIONES y las 6 tablas CAP_*. Confirmado también con la anon
-- key pura (mismo error), y confirmado que la función no deja residuo:
-- al no tener manejo de excepciones, la transacción implícita de la
-- llamada RPC revierte todo por completo ante el error.
--
-- FIX: cambiar p_id/v_id de uuid a text. Body idéntico al de
-- 20260818_inspecciones_atomic_save.sql, con exactamente 3 cambios:
--   (a) p_id uuid -> p_id text (firma)
--   (b) v_id uuid; -> v_id text; (DECLARE)
--   (c) v_id := extensions.uuid_generate_v4(); ->
--       v_id := extensions.uuid_generate_v4()::text; (modo creación)
-- Ninguna otra línea fue tocada -- las comparaciones/inserts que ya usaban
-- v_id quedan textualmente iguales y pasan a ser correctas automáticamente
-- porque v_id ahora es text.
--
-- CREATE OR REPLACE no alcanza acá porque el cambio de tipo de p_id es un
-- cambio de firma -- requiere DROP + CREATE (mismo patrón ya usado para
-- fn_listar_padron_socios). Eso resetea los grants a los defaults de
-- Postgres (EXECUTE a PUBLIC), así que se agrega un REVOKE explícito antes
-- del GRANT para no reabrir ese acceso por defecto.
--
-- No se decide acá si `anon` debe seguir teniendo EXECUTE sobre esta
-- función -- esa decisión es de Fase C Paso 2, una vez se endurezca el RLS
-- a nivel de tabla.

BEGIN;

DROP FUNCTION IF EXISTS public.fn_guardar_inspeccion_completa(uuid, text, text, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb);

CREATE FUNCTION public.fn_guardar_inspeccion_completa(
  p_id text,
  p_organizacion text,
...
BEGIN
  ...
  ELSE
    -- ── Modo creación ─────────────────────────────────────────────────
    v_id := extensions.uuid_generate_v4()::text;
    v_created := true;
...
-- DROP + CREATE resetea los grants a los defaults de Postgres (EXECUTE a
-- PUBLIC) -- se revoca explícitamente antes de re-otorgar a los mismos
-- destinatarios de antes.
REVOKE ALL ON FUNCTION public.fn_guardar_inspeccion_completa(

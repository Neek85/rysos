...
5. [x] Reenviar "Send password recovery" a Eduardo y Dante (Dashboard de Supabase, manual),
   confirmado "Delivered" en Resend.
6. [x] Confirmación real del usuario: los dos (Eduardo y Dante) completan el formulario y
   llegan a `/dashboard/mapa` con sesión válida (ver hallazgo del 404 de `/dashboard` a secas,
   `specs/redirect_dashboard_default.md`, corregido aparte).
7. [x] `docs/ESTADO_PROYECTO.md` — entrada de cierre del incidente completo, atribución
   Claude/Cowork de punta a punta.
8. [x] Commit + push a `staging` (`15be571`).

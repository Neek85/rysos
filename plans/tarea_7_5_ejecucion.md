...
2. **Desarrollo del Script ETL (`scripts/etl_drive_to_supabase.py`):**
   - `discover_packages(inbox_dir)`: localizar `.zip` bajo `INBOX/*/*.zip`.
   - `get_org_id_from_path(zip_path)`: derivar `ID_Organizacion` de la carpeta padre.
   ...
   - `run(execute_move)`: recorre todos los paquetes descubiertos en `INBOX/` y los procesa.

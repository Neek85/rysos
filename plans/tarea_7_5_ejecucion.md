...
2. **Desarrollo del Script ETL (`scripts/etl_drive_to_supabase.py`):**
   - `discover_packages()`: localizar `.zip` bajo `RYZOS_CLIENTES/*/RYZOS_INBOX/*.zip` (jerarquía tenant-first).
   ...
   - `build_archive_destination(zip_path, org_id, timestamp)`: calcular ruta `RYZOS_CLIENTES/{org_id}/RYZOS_ARCHIVE/PROCESADO_YYYYMMDD_HHMMSS_{filename}.zip`.
   ...
   - `run(execute_move)`: recorre todos los paquetes descubiertos bajo `RYZOS_CLIENTES/*/RYZOS_INBOX/` y los procesa.
...
- El paquete NO se archiva si el proceso falla antes de completar la inserción y carga de evidencias (se mantiene en `RYZOS_CLIENTES/{ID_Organizacion}/RYZOS_INBOX/` para reintento).

...
import { resolveAuthRole } from '@/lib/auth/resolveAuthRole'
...
/**
 * RBAC (specs/smoke_test_fase_d_paso2.md, AI_STATE.md 2026-09-08) --
 * la matriz de permisos (specs/login_real_organizacion_rol.md §5) dice
 * `admin`/`tecnico_campo`: Sí, `auditor_qc`: Solo lectura para
 * /dashboard/inspecciones. Hasta esta migración, `saveInspeccion` no
 * tenía ningún chequeo de rol -- el RLS real de INSPECCIONES/CAP_*
 * (ADR-033) solo exige organización, nunca rol. Mismo criterio que
 * `assertAdminRole` (lib/actions/sociosActions.js): capa de aplicación,
 * fail-fast de UX -- la autoridad real es el trigger de base de datos
 * (ver migración `enforce_inspecciones_role_trigger`).
 */
async function assertInspeccionWriteRole(supabase) {
  const rol = await resolveAuthRole(supabase)
  if (rol !== 'admin' && rol !== 'tecnico_campo') {
    throw new InspeccionError('Esta acción requiere el rol admin o tecnico_campo.')
  }
}
...
function mapToFormValues(merged) {
  ...
  return {
    ID_Socio: s(merged.ID_Socio),
    ID_Parcela: s(merged.ID_Parcela),
...
function payloadInspeccion(v) {
  return {
    ID_Socio: trim(v.ID_Socio),
    ID_Parcela: trim(v.ID_Parcela),
...
/**
 * Crea o actualiza una inspección completa (INSPECCIONES + 6 CAP_*) de
 * forma atómica, vía la función RPC transaccional
 * public.fn_guardar_inspeccion_completa (supabase/migrations/
 * 20260818_inspecciones_atomic_save.sql, tipos corregidos en
 * 20260903045407_fix_tipo_id_inspeccion.sql). Antes esto eran 7 llamadas
 * REST independientes (Promise.all) sin transacción de base de datos —
 * una falla a mitad de camino dejaba registros huérfanos/parciales. La
 * verificación multi-tenant client-side (abajo) es solo fail-fast antes
 * del round-trip de red — la función en sí NO repite esa verificación
 * contra la sesión real: es SECURITY INVOKER (sin cláusula explícita) y
 * solo compara sus propios parámetros `p_organizacion`/
 * `p_existing_organizacion` entre sí, ninguno derivado del llamador. La
 * autoridad real es el RLS de INSPECCIONES/CAP_* (ADR-033, `WITH CHECK
 * "ID_Organizacion" = auth_org_id()`), que aplica porque la función
 * corre con el rol de quien la invoca — nunca confiar solo en el chequeo
 * del cliente ni asumir que la función valida por su cuenta.
 *
 * `organizationId` se valida contra `existingOrganizationId` (la
 * organización real del registro que se está editando, resuelta al
 * cargar vía fetchInspeccionDetalle) antes de escribir — nunca contra un
 * valor que el propio formulario pueda enviar. En creación,
 * `existingOrganizationId` simplemente queda `null` (estado inicial); la
 * comparación de abajo está gateada por `id`, así que nunca se ejecuta
 * en ese modo.
 */
export async function saveInspeccion(supabase, values, { id, organizationId, existingOrganizationId }) {
  await assertInspeccionWriteRole(supabase)
  ...
  const { data, error } = await supabase.rpc('fn_guardar_inspeccion_completa', {
    p_id: id ?? null,
    p_organizacion: organizationId,
    p_existing_organizacion: existingOrganizationId ?? null,
    p_inspeccion: payloadInspeccion(values),
    p_socio: payloadSocio(values),
    p_mic: payloadMic(values),
    p_conservacion: payloadConservacion(values),
    p_bienestar: payloadBienestar(values),
    p_riesgos: payloadRiesgos(values),
    p_gestion: payloadGestion(values),
  })
  if (error) throw error

...
/**
 * Deriva el texto/estado del badge de Áreas Naturales Protegidas a partir
 * del campo `anp` que devuelve fn_validar_topologia_eudr (motor real
 * conectado en specs/motor_prevalidacion_satelital_anp_bosque.md) — mismo
 * criterio exacto que describeDeforestationBadge arriba: `ok:null`
 * cuando `EUDR_AREAS_PROTEGIDAS` sigue vacía (nunca inventa un
 * veredicto), `ok:false` ante cualquier superposición real (tolerancia
 * cero, sin importar el porcentaje).
 */
export function describeAnpBadge(anp) {
  if (!anp?.disponible) {
    return { ok: null, label: '🛰️ ANP: sin datos (no integrado)' }
  }
  if (anp.alerta_anp) {
    const nombre = anp.intersecciones?.[0]?.nombre
    return { ok: false, label: nombre ? `⚠ Superpone ANP: ${nombre}` : '⚠ Superpone Área Natural Protegida' }
  }
  return { ok: true, label: '✓ Sin superposición con ANP' }
}

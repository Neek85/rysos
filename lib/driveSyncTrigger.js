...
// code=3221225794 (NTSTATUS 0xC0000142, STATUS_DLL_INIT_FAILED — una DLL
// requerida por el "python" resuelto por spawn() en ese proceso concreto
// SÍ se encontró y cargó, pero su rutina de inicialización falló) y CERO
// bytes en stdout/stderr, porque el proceso jamás ejecutó ninguna línea.
// `(stderr || stdout).slice(-2000)` sobre dos strings vacíos da `""` — la
// información real (que el proceso murió sin arrancar, con qué código) se
// descartaba en vez de reportarse.
const KNOWN_EMPTY_OUTPUT_EXIT_CODES = {
  3221225794:
    'Windows encontró y cargó una DLL requerida por el intérprete de Python resuelto, pero su ' +
    'rutina de inicialización falló (NTSTATUS 0xC0000142, STATUS_DLL_INIT_FAILED) — el proceso ' +
    'nunca llegó a arrancar. En este proyecto se observó ligado a un proceso `next dev` de larga ' +
    'duración; reiniciarlo (matar node, borrar .next, `npm run dev` de nuevo) lo resolvió.',
}

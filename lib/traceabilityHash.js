...
// campos que SÍ existen en el payload real de buildTracesPayload() (schema
// vw_monitoreo_web) y son PII por el mismo motivo aunque no coincidan
// textualmente con esos nombres: `productor` es un código interno que en la
// práctica suele resolver a un ID_Socio o a un nombre libre (ver COALESCE
// en la vista), `productor_nombre` (agregado en ADR-017 para
// ProducerName del GeoJSON oficial) es directamente
// PADRON_SOCIOS.socio_nombre_completo ya resuelto — nombre real de
// persona, igual de sensible — e `id_parcela` es el identificador interno
// crudo (con frecuencia UUID) que el resto del código ya trata como no
// apto para exponer (ver sanitizeCode en components/gis/MapDashboard.jsx).
...
// -- NEXT_PUBLIC_ requerido porque este módulo corre también client-side
// (app/dashboard/lotes/page.jsx es 'use client'). Fallback al dominio de
// producción real, no a localhost: un despliegue con la variable
// olvidada debe fallar hacia la URL pública correcta, no hacia una que
// ningún usuario externo puede resolver.
const TRACE_BASE_URL = `${process.env.NEXT_PUBLIC_APP_URL || 'https://ryzosagri.com'}/trace/`

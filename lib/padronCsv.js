 ...
 * certificaciones activas para reconocer sus columnas dinámicas en el
 * CSV, y RECHAZA el archivo COMPLETO (lanza, no marca fila por fila) si
 * encuentra alguna columna que no matchea ningún campo fijo conocido ni
 * el nombre de una certificación activa — nunca la ignora en silencio.
 * Sin `supabase`, esta validación específica se omite (mismo criterio de
...
export async function validateSocioRows(rows, supabase, organizationId) {
  ...
  if (supabase) {
    ...
    const unrecognized = findUnrecognizedSocioColumns(rows, reverseMap)
    if (unrecognized.length > 0) {
      throw new Error(
        `El archivo tiene columna(s) no reconocida(s): ${unrecognized.join(', ')}. Revisá el nombre exacto contra la plantilla descargada o el catálogo de certificaciones activo.`
      )
    }
  }
...
/** Ver validateSocioRows — mismo criterio, `supabase`/`organizationId` opcionales. */

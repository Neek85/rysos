...
import { getDepartamentos } from './ubigeoData.js'
...
const SOCIO_TEMPLATE_EXAMPLE = {
  ...
  socio_dni: '12345678',
  socio_genero: 'Hombre',
  // mejoras_importador_padron_masivo.md ronda 3: formato D/M/AAAA o
  // M/D/AAAA (ver fechaNacimiento en lib/validations/socios.js) -- ya no
  // acepta guiones ISO. "14/5/1980" es válido en cualquiera de las 2
  // lecturas (14 y 5 caben como día o mes), no ambigua a propósito.
  socio_fecha_nacimiento: '14/5/1980',
...
// archivo nuevo, ver corrección de premisa en el spec sección 0. Solo
// Departamento se valida acá (25 filas, alta confianza según el propio
// _meta del JSON); Provincia queda pendiente a propósito -- aunque el
// catálogo también tiene provincias completas, no fue pedido para esta
// ronda (spec sección 1f).
const DEPARTAMENTOS_NORMALIZADOS = new Set(getDepartamentos().map(normalizeForCatalogMatch))

/**
 * Mejoras importador padrón masivo (spec sección 1f, ronda 3): rechazo de
 * FILA (no de archivo completo) si `socio_departamento` tiene un valor no
 * vacío que no matchea ningún departamento del catálogo real. Vacío sigue
 * siendo válido (campo opcional en socioSchema, sin cambios acá).
 *
 ...
 * la lista" que guarda un valor de texto libre FUERA del catálogo (ver su
 * propio comentario: "un distrito real ausente del dataset NUNCA bloquea
 * el alta de un socio real"). Si esta validación viviera en `socioSchema`,
 * elegir "Otro" en el formulario manual rompería el guardado -- exactamente
 * el caso que esa opción existe para evitar. Por eso el chequeo vive acá,
 * específico del importador masivo, no en el schema compartido.
 */
function applyDepartamentoCatalogCheck(results, normalizedRows) {
  normalizedRows.forEach((row, i) => {
    const value = (row.socio_departamento ?? '').toString().trim()
    if (!value) return
    if (!DEPARTAMENTOS_NORMALIZADOS.has(normalizeForCatalogMatch(value))) {
      results[i].valid = false
      results[i].errors.push(`El Departamento "${value}" no es un departamento válido de Perú.`)
    }
  })
  return results
}
 ...
 * (rechazo de fila, no de archivo); `socio_fecha_nacimiento` valida
 * formato D/M/AAAA o M/D/AAAA; `socio_departamento` se valida acá (no en
 * `socioSchema`, ver `applyDepartamentoCatalogCheck`) contra el catálogo
 * real de 25 departamentos de `lib/ubigeoData.js`.
 *
...
export async function validateSocioRows(rows, supabase, organizationId) {
  ...
  applyDepartamentoCatalogCheck(results, normalizedRows)
...
export async function validateParcelaRows(rows, supabase, organizationId) {
  ...
  if (supabase && organizationId) {
    const missingSocios = await applyParcelaDbChecks(results, normalizedRows, supabase, organizationId)
    if (missingSocios.size > 0) {
      missingSocioWarnings = [...missingSocios.entries()].map(
        ([socioId, lines]) => `El Código de Socio "${socioId}" no existe en la organización activa (fila(s) ${lines.join(', ')}).`
      )
    }
  }

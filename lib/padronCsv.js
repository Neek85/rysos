...
import { getDepartamentos, getProvincias } from './ubigeoData.js'
...
// validaba solo Departamento; ronda 4 (mejoras_importador_padron_masivo.md
// sección 7) extiende a Provincia -- el catálogo ya tenía las ~196
// provincias completas, no hizo falta agregar ningún dato nuevo, solo usar
// getProvincias() (lib/ubigeoData.js) que ya existía.
//
// Mapa normalizado -> nombre CANÓNICO (no solo un Set) porque
// getProvincias(departamentoNombre) necesita el nombre EXACTO del
// catálogo para resolver sus provincias -- si el archivo trae
// "CAJAMARCA" hay que resolverlo a "Cajamarca" antes de poder llamar
// getProvincias.
const DEPARTAMENTOS_CANONICOS_POR_NORMALIZADO = new Map(getDepartamentos().map((d) => [normalizeForCatalogMatch(d), d]))

/**
 * Mejoras importador padrón masivo (spec sección 1f/7, rondas 3-4):
 * rechazo de FILA (no de archivo completo) si `socio_departamento` no
 * matchea ningún departamento real, o si `socio_provincia` no pertenece
 * al `socio_departamento` declarado EN LA MISMA FILA (o no existe en el
 * catálogo). Ambos campos vacíos siguen siendo válidos (opcionales en
 * socioSchema, sin cambios acá) -- una Provincia con valor pero sin
 * Departamento en la misma fila SÍ se rechaza: no hay forma de validar la
 * pertenencia sin saber a qué departamento pertenece.
 *
 ...
 * la lista" que guarda un valor de texto libre FUERA del catálogo, tanto
 * para Departamento como para Provincia (ver su propio comentario: "un
 * distrito real ausente del dataset NUNCA bloquea el alta de un socio
 * real"). Si esta validación viviera en `socioSchema`, elegir "Otro" en
 * el formulario manual rompería el guardado -- exactamente el caso que
 * esa opción existe para evitar. Por eso el chequeo vive acá, específico
 * del importador masivo, no en el schema compartido.
 */
  ...
  normalizedRows.forEach((row, i) => {
    ...
    if (departamentoValue) {
      ...
      if (!departamentoCanonico) {
        results[i].valid = false
        results[i].errors.push(`El Departamento "${departamentoValue}" no es un departamento válido de Perú.`)
      }
    }
    ...
    const provinciasNormalizadas = getProvincias(departamentoCanonico).map(normalizeForCatalogMatch)
    if (!provinciasNormalizadas.includes(normalizeForCatalogMatch(provinciaValue))) {
      results[i].valid = false
 ...
 * `socio_departamento`/`socio_provincia` se validan acá (no en
 * `socioSchema`, ver `applyUbigeoCatalogChecks`) contra el catálogo real
 * de `lib/ubigeoData.js` -- Provincia además valida pertenencia al
 * Departamento de la misma fila (ronda 4).
 *

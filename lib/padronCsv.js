 ...
 */
export function buildSocioTemplateCsv(sampleSocioId) {
  const example = { ...SOCIO_TEMPLATE_EXAMPLE, ID_Socio: sampleSocioId || SOCIO_TEMPLATE_EXAMPLE.ID_Socio }
  return arrayToCsv([example], SOCIO_TEMPLATE_COLUMNS, SOCIO_FIELD_LABELS)
}
...
export async function downloadSocioTemplate(supabase, organizationId) {
  ...
  triggerCsvDownload('Plantilla_Socios.csv', buildSocioTemplateCsv(sampleSocioId))
}

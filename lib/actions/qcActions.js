...
//
// PILOTO CAMINO 1 (2026-09-05, ver
// docs/adr/ADR-035-piloto-camino-1-rls-sesion-qc-atributos-geometria.md):
// `updateQcRecordAttributes`/`updateQcRecordGeometry` ahora corren con la
// sesión real del usuario (`createSessionServerClient`), no con Service
// Role Key — el RLS real de ADR-034 (`"ID_Organizacion" = auth_org_id()`)
// es la autoridad, no un bypass.
//
// FASE D (2026-09-04, ver
// docs/adr/ADR-039-fase-d-qc-aprobar-rechazar-roles-rls.md):
// `approveQcRecord`/`rejectQcRecord` también migran a sesión real. Lo
// que las bloqueaba (motivo original en ADR-035) era que el RLS por
// organización por sí solo no distingue `admin`/`auditor_qc` de
// `tecnico_campo` — eso ya no es un bloqueo: el trigger
// `fn_enforce_qc_approval_roles` (la migración de este ADR) exige esos 2
// roles en Postgres mismo cuando `estado_revision` cambia de verdad,
// así que el RLS de sesión ya es seguro para estas 2 funciones también.
...
// specs/revertir_aprobado_a_qc.md — sin parámetro `motivo`: a diferencia
// de rejectQcRecord, revertir no persiste ningún motivo en la fila (el
// log de auditoría, si se dio un motivo, lo agrega aparte desde
// page.jsx, igual que ya hace para Aprobar/Rechazar).
export async function reopenQcRecord(record, organizationId) {
  const supabase = await createSessionServerClient()
  await reopenRecord(supabase, record, organizationId)
  return { ok: true }
}

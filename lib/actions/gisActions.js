...
// invocado en la práctica desde /dashboard/qc (CargaEspacialModal.jsx/
// VectorEditorTools.jsx), no desde /dashboard/mapa como decía este
// comentario originalmente — ver specs/gis_ingestor_web.md. Primer path
// de escritura del frontend hacia EUDR_MONITOREO/EUDR_USO_SUELO/
// EUDR_INSTALACIONES (antes solo el ETL de Python y QGIS Desktop
// escribían esas 3 tablas).
//
// PILOTO CAMINO 1, FASE A.3 (ver
// docs/adr/ADR-038-fase-a3-rls-sesion-gis-ingestor.md): las 3 ramas EUDR
// de uploadGeoSpatialFeature corren con la SESIÓN REAL del usuario
// (`createSessionServerClient`), no con Service Role Key -- el RLS real
// de ADR-034 (`"ID_Organizacion" = auth_org_id()`) es la autoridad, no
// un bypass. La organización se resuelve server-side vía
// `supabase.rpc('auth_org_id')`, NUNCA desde el parámetro `organizationId`
// que manda el cliente (ver el comentario dentro de la función) -- esto
// también corrige un bug real y actual: con las 3 tablas EUDR vacías,
// `resolveOrganizationId(records)` en app/dashboard/qc/page.jsx devuelve
// `null`, y antes de esta migración eso bloqueaba la escritura para
// cualquier organización antes de cualquier llamada de red. La rama
// `PADRON_PARCELAS` (delega en `createParcela`, ya migrada en ADR-036)
// sigue usando el parámetro `organizationId` del cliente -- no se tocó.
//
...
export async function uploadGeoSpatialFeature(targetTable, feature, organizationId, fieldOverrides = {}) {
  ...
  if (targetTable === 'PADRON_PARCELAS') {
    // Único caso que todavía usa `organizationId` (el parámetro que manda
    // el cliente) -- createParcela (lib/actions/sociosActions.js) ya está
    // migrada a sesión real (ADR-036) y hace su propia validación
    // multi-tenant contra ese valor; no se tocó acá.
    assertOrganizacion(organizationId)
    return createParcela({ ...feature.properties, ...fieldOverrides }, organizationId, feature.geometry)
  }

  // Fase A.3 (ADR-038): las 3 ramas EUDR de abajo YA NO usan
  // `organizationId` (el parámetro del cliente) para nada -- la
  // organización real se resuelve acá, server-side, vía la sesión
  // (`auth_org_id()`), la misma función que el RLS de ADR-034 usa como
  // autoridad en `rls_write_eudr_*`. `organizationId` queda en la firma
  // de esta función solo por la rama PADRON_PARCELAS de arriba.
  const supabase = await createSessionServerClient()
  const { data: sessionOrgId, error: orgError } = await supabase.rpc('auth_org_id')
  if (orgError) throw orgError
  if (!sessionOrgId) {
    throw new GisActionError('No se pudo determinar la organización de la sesión activa.')
  }

  if (targetTable === 'EUDR_MONITOREO') {
    await assertSocioActivoOSinValor(supabase, fieldOverrides.ID_Socio, sessionOrgId)
    await assertParcelaActivaOSinValor(supabase, fieldOverrides.ID_Parcela_Fija, sessionOrgId)
    await insertEudrCoreRecord(supabase, targetTable, geometryWkt, sessionOrgId, {
  ...
  if (targetTable === 'EUDR_USO_SUELO') {
    ...
    // ADR-019 -- ahora contra `sessionOrgId`, no `organizationId`). Pero
    // lo que se GUARDA en la columna id_parcela no es ese código: es el
    // identificador técnico real del Monitoreo padre
    ...
    await assertParcelaActivaOSinValor(supabase, fieldOverrides.id_parcela, sessionOrgId)
    const qfieldRelationId = await resolveQfieldRelationId(supabase, fieldOverrides.id_parcela, sessionOrgId)
    await insertEudrCoreRecord(supabase, targetTable, geometryWkt, sessionOrgId, {

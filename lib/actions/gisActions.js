 ...
 * assertSocioExists (lib/actions/sociosActions.js): se trae
 * `ID_Organizacion` sin filtrarla en la query y se compara con
 * `orgIdsMatch` (tolerante a mayúsculas/espacios) en vez de un `.eq()`
 * crudo, para no duplicar 2 formas distintas de comparar organización en
 * el mismo módulo GIS.
 */
async function assertSocioActivoOSinValor(supabase, socioId, organizationId) {
  ...
  if (!data || !orgIdsMatch(data.ID_Organizacion, organizationId) || data.activo !== true) {

...
// por PII).
//
// PILOTO CAMINO 1, FASE A.1 (2026-09-05, ver
// docs/adr/ADR-036-migracion-parcial-camino-1-sociosactions.md): split
// deliberado dentro de este mismo archivo.
//   - `updateParcela`/`createParcela`/`deactivateParcela`/`deactivateSocio`
//     ahora corren con la SESIÓN REAL del usuario (`createSessionServerClient`)
//     — el RLS real de ADR-034 en PADRON_SOCIOS/PADRON_PARCELAS
//     (`"ID_Organizacion" = auth_org_id()`) es la autoridad, no un bypass.
//   - `createSocio`/`updateSocio` (vía `syncSocioCertificaciones`) y
//     `resolveSocioCertFlags` SIGUEN con Service Role Key
//     (`getSupabaseServerClient`) a propósito — bloqueados por 2 gaps
//     reales, no un descuido: `fn_crear_socio_con_certificaciones` no
//     tiene `GRANT EXECUTE` para `authenticated`, y `SOCIO_CERTIFICACIONES`/
//     `CERTIFICACIONES_CATALOGO` no tienen ninguna política RLS para
//     `authenticated` (solo `anon SELECT`) — Fase A.2, tarea aparte.
//
// Para las funciones que SIGUEN con Service Role Key: como esa llave
// bypasea RLS, el aislamiento multi-tenant es responsabilidad explícita
// de este archivo — mismo patrón que lib/inspeccionesActions.js::saveInspeccion
...
// nunca contra un valor que el propio formulario pueda inventar. Para las
// 4 funciones ya migradas, esa misma validación de aplicación se mantiene
// como defensa en profundidad (ver ADR-036, "Nota de comportamiento") —
// el RLS real es ahora la autoridad de fondo, no el único guardia.

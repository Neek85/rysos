...
export async function deactivateSocio(socioId, organizationId) {
  ...
  const { error } = await supabase.from('PADRON_SOCIOS').update({ activo: false }).eq('ID_Socio', socioId)
  if (error) throw error
...
export async function deactivateParcela(parcelaId, socioId, organizationId) {
  ...
  const { error } = await supabase.from('PADRON_PARCELAS').update({ activo: false }).eq('ID_Parcela_Fija', parcelaId)
  if (error) throw error

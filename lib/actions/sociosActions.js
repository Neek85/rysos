...
/**
 * RBAC (specs/rbac_webgis_padron.md) — la matriz de permisos ya
 * confirmada (specs/login_real_organizacion_rol.md §5) dice
 * "Solo lectura" para tecnico_campo/auditor_qc en /dashboard/socios:
 * ninguna mutación de este archivo (alta, edición, baja) es exclusiva
 * de admin. El ocultamiento de botones en la UI (page.jsx,
 * ParcelaFormModal.jsx) es solo UX — esta función es la capa real,
 * exigida antes de cualquier escritura, así que invocar estas Server
 * Actions directo (sin pasar por la UI) también queda bloqueado.
 */
async function assertAdminRole(supabase) {
  const rol = await resolveAuthRole(supabase)
  if (rol !== 'admin') {
    throw new SocioActionError('Esta acción requiere el rol admin.')
  }
}
...
export async function createSocio(values, organizationId) {
  ...
  await assertAdminRole(supabase)

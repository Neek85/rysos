...
// Mejoras importador padrón masivo (spec sección 9, ronda 6): antes era
// `z.enum(['Sí', 'No'])` -- se cambió a `.refine()` (mismo idioma que ya
// usa `fechaNacimiento`/el refine de suma de hectáreas más abajo) para
// poder fijar un mensaje de error legible propio ("Debe ser Sí o No") sin
// pelear con el mensaje genérico de enum de Zod ("Invalid enum value...").
// El valor YA llega normalizado a 'Sí'/'No' exactos antes de este punto
// (ver normalizeSiNo en lib/padronCsv.js, corre en validateSocioRows
// antes de socioSchema.safeParse) -- este refine es la última línea de
// defensa para un valor que ni siquiera normalizado matchea sí/no.
const siNo = z
  .string()
  .optional()
  .nullable()
  .or(z.literal(''))
  .refine((val) => !val || val === 'Sí' || val === 'No', { message: 'Debe ser Sí o No' })
...
const dniRequerido = z
  .string({ required_error: 'Este campo es obligatorio' })
  .min(1, 'Este campo es obligatorio')
  .regex(/^\d{8}$/, 'El DNI debe tener 8 dígitos')

// Celular peruano: 9 dígitos (formato de línea móvil nacional, ej. 9XXXXXXXX).
// Confirmado sin cambios de comportamiento en la ronda 3 (punto 1d) --
// ya validaba esto desde antes; ronda 6 solo ajusta el texto del mensaje
// ("El celular...", antes "Celular...") para que se lea natural con el
// label humano antepuesto (ver formatZodIssues en lib/padronCsv.js).
const celular = z
  .string()
...
const fechaNacimiento = z
  ...
  .refine(
    ...
    { message: 'La fecha debe tener el formato M/D/AAAA (ej: 4/29/1986)' }
  )
...
export const socioSchema = z.object({
  ...
  ID_Socio: z.string({ required_error: 'Este campo es obligatorio' }).min(1, 'Este campo es obligatorio'),
  codigo_finca: str,
  socio_nombre_completo: z.string({ required_error: 'Este campo es obligatorio' }).min(1, 'Este campo es obligatorio'),
...
// campo técnico.
//
// Ronda 7 (2026-09-01): corrección del label de `hip` -- el texto EXACTO
// real es "Ha. Inverna/Pasto" (abreviado), NO "Ha. Invernadero/Pasto"
// (palabra completa) como se había puesto en la ronda 3 -- corrección de
// una corrección anterior, con evidencia directa del archivo real del
// usuario. `hrp` ("Ha. Rastrojo/Purma") NO cambia en esta ronda -- no fue
// parte de este pedido. Los 2 labels viejos ("Ha. Infraestructura
// Productiva" de antes de la ronda 3, y "Ha. Invernadero/Pasto" de la
// ronda 3) quedan como alias reconocidos en `lib/padronCsv.js`
// (`PARCELA_REVERSE_LABELS`) -- ver ahí, no acá: `HECTARE_FIELDS` sigue
// siendo la única fuente para el label CANÓNICO (el que se exporta/
// muestra), los alias de reconocimiento en import son responsabilidad de
// la capa de importación, no del schema.
export const HECTARE_FIELDS = [
  { field: 'hcp', label: 'Ha. En Producción' },
  { field: 'hcc', label: 'Ha. En Crecimiento' },
  { field: 'ho', label: 'Ha. Otros' },
  { field: 'hip', label: 'Ha. Inverna/Pasto' },
...
export const parcelaSchema = z
  .object({
    ID_Parcela_Fija: z.string({ required_error: 'Este campo es obligatorio' }).min(1, 'Este campo es obligatorio'),
    // Mensaje contextual al formulario manual (dropdown de socio) --
    // deliberadamente NO se unificó con "Este campo es obligatorio" (ronda
    // 6): en la importación CSV este campo casi nunca aparece vacío (rompe
    // la relación Parcela->Socio), y cuando aparece, applyParcelaDbChecks
    // ya da un mensaje más específico aparte; en el formulario manual
    // "Selecciona un socio" sigue siendo más claro que el genérico porque
    // ahí sí hay un dropdown real que seleccionar.

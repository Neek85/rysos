...
// Fecha de nacimiento: acepta D/M/AAAA o M/D/AAAA indistintamente, SIN
// exigir ceros de relleno -- decisión confirmada con el usuario 2026-08-31
// (ver specs/mejoras_importador_padron_masivo.md sección 0): la primera
// carga real (COOP-AROMAS-VALLE, 618 socios) trae fechas en M/D/AAAA sin
// padding (ej. "4/29/1986" = 29 de abril -- inequívoco, 29 no puede ser
// mes), NO en DD/MM/AAAA como pedía el prompt original; un regex DD/MM/AAAA
// estricto habría rechazado 478 de 544 fechas reales (88%). No se distingue
// posicionalmente cuál parte es día y cuál es mes -- el campo es texto
// libre de display, nada lo parsea como fecha real hoy en ningún lugar del
// repo (confirmado) -- se acepta si existe AL MENOS UNA interpretación
// válida (la parte ≤31 puede ser el día, la parte ≤12 puede ser el mes).
...
const fechaNacimiento = z
  ...
  .refine(
    (val) => {
      ...
      return a >= 1 && b >= 1 && Math.max(a, b) <= 31 && Math.min(a, b) <= 12
    },
    { message: 'Formato de fecha inválido (usar D/M/AAAA, ej. 29/4/1986 o 4/29/1986)' }
  )

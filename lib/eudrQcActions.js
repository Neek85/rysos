...
export async function checkSocioParcelaOrganizacion(supabase, record) {
    ...
    if (data?.ID_Organizacion && data.ID_Organizacion !== record.ID_Organizacion) {
      return {
        ...
        mensaje:
          ...
          `organización "${data.ID_Organizacion}", no a "${record.ID_Organizacion}" — probablemente un dato de ` +
  ...
  if (record.tabla_origen === 'EUDR_MONITOREO' && record.id_monitoreo) {
    ...
    if (socioId) {
      ...
      if (socio?.ID_Organizacion && socio.ID_Organizacion !== record.ID_Organizacion) {
        return {
          tieneConflicto: true,
          mensaje:
            `No se puede aprobar${fechaTexto}: el Código de Socio "${socioId}" pertenece a la organización ` +
            `"${socio.ID_Organizacion}", no a "${record.ID_Organizacion}" — probablemente un dato de prueba o ` +

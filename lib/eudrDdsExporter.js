...
/**
 * Genera el payload TRACES UE y dispara la descarga de dos archivos en el
 * navegador: DDS_TRACES_[ORG]_[FECHA].json (payload completo) y
 * DDS_GEOMETRIAS_[ORG]_[FECHA].geojson (solo la FeatureCollection). Lanza
 * EUDRValidationError si alguna regla EUDR no se cumple — el caller decide
 * cómo mostrarlo (toast/alert).
 */
export function exportTracesDDS(records, organizationId) {

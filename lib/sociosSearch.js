 ...
 * un valor truthy, se usa DIRECTO como `organizationId` y el probe/
 * fallback de arriba se saltea por completo -- el caller
 ...
 * `lib/actions/organizacionesActions.js`) antes de pasarlo acá; esta
 * función no repite esa verificación. `undefined`/`null` (el default)
 * preserva el comportamiento normal sin ningún cambio.
 */
export async function fetchSocios(
  ...
  let organizationId = organizationIdOverride || null
  if (!organizationId) {
    ...
    organizationId = orgProbe?.[0]?.ID_Organizacion
    if (!organizationId) {

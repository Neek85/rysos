...
- **Estructura de Storage:** Las fotografías adjuntas se suben al bucket `evidencias-eudr` bajo la ruta `{ID_Organizacion}/{id_monitoreo}/{nombre_foto}`.
  - **Nota de reconciliación:** el bucket `evidencias_eudr` (guion bajo) ya existe y es el usado por el ETL de Fase 2 (`scripts/etl_qfield_ingest.py`). Esta tarea usa el nombre `evidencias-eudr` (guion medio) tal como fue especificado. Antes de desplegar a producción, confirmar con el usuario si ambos ETL deben apuntar al mismo bucket físico o si `evidencias-eudr` es intencionalmente un bucket nuevo/distinto.
...
- [ ] Cada fotografía encontrada en el paquete genera una ruta de Storage con el patrón `{ID_Organizacion}/{id_monitoreo}/{nombre_foto}` bajo el bucket `evidencias-eudr`.

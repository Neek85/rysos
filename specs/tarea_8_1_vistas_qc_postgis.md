...
- **`cumple_eudr` (agregado en Tarea 9.3, widget de métricas):** columna de `EUDR_MONITOREO` que nunca se había propagado a las tres vistas. Se agregó a `vw_monitoreo_poligonos`, `vw_monitoreo_puntos` y `vw_monitoreo_web` con el mismo patrón que el resto de campos exclusivos de `EUDR_MONITOREO`: valor real en la rama `EUDR_MONITOREO`, `NULL::text` en las ramas `EUDR_USO_SUELO`/`EUDR_INSTALACIONES` (esas tablas no tienen este campo). Consumido por `components/gis/EudrStatsWidget.jsx` para calcular el porcentaje de cumplimiento EUDR — ese cálculo se hace sobre las filas donde `cumple_eudr IS NOT NULL` (solo `EUDR_MONITOREO`), no sobre el total de la vista, para no diluir el porcentaje con filas donde el campo no aplica.
- **`parcela_nombre` / `area_ha` (agregado en Tarea 9.4, tooltips/popups del mapa):** ninguna de las 3 tablas EUDR tiene estos campos — viven en `PADRON_PARCELAS` (`parcela_nombre`, `totalh`). Se agregó un `LEFT JOIN public."PADRON_PARCELAS" ON "ID_Parcela_Fija" = "ID_Parcela_Fija"` **solo en `vw_monitoreo_web`** (no en `vw_monitoreo_poligonos`/`vw_monitoreo_puntos`, que son para auditoría QGIS y no necesitan el nombre legible), replicando el mismo patrón ya usado en `view_eudr_dashboard_aprobados` (Fase 1). `totalh` se expone con el alias `area_ha`. `LEFT JOIN` (no `INNER`) porque una parcela puede no tener fila aún en `PADRON_PARCELAS`. El aislamiento multi-tenant del join no necesita repetir la condición de `ID_Organizacion` — ya lo cubre la RLS propia de `PADRON_PARCELAS` (`rls_select_padron_parcelas`).
- **`parcela_codigo` + productor de respaldo (agregado en Tarea 9.5, `supabase/migrations/20260817_refine_vw_monitoreo_web.sql`):** `"ID_Parcela_Fija"` es un identificador interno (con frecuencia con forma de UUID), no el código de negocio legible que se venía mostrando en el mapa. Se agregó `pp.parcela_codigo` (mismo JOIN a `PADRON_PARCELAS` ya existente) como el código real a mostrar; el filtrado defensivo de cualquier UUID remanente vive del lado del cliente (`sanitizeCode` en `MapDashboard.jsx`). También se agregó `COALESCE(src.productor, mon.productor)` con un `LEFT JOIN LATERAL` que busca la visita `EUDR_MONITOREO` más reciente sobre la misma parcela (mismo `"ID_Parcela_Fija"` + `"ID_Organizacion"`) para rellenar el productor en filas `EUDR_USO_SUELO`/`EUDR_INSTALACIONES` (que nunca tienen ese campo propio) — sin depender de una relación `PADRON_PARCELAS → PADRON_SOCIOS` no confirmada. Si la parcela nunca tuvo una visita registrada, el productor queda `NULL` de todas formas.

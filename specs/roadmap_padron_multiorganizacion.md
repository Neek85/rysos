...
- Paso 4, certificaciones normalizadas (ADR-027) — completado (código y migración listos, commit pendiente de push). 5 rondas de auditoría (specs/padron_certificaciones_normalizado.md) cerraron un contrato de datos DISTINTO del boceto original de la sección "1. Certificaciones" de este roadmap: 5 tablas (no 3), sin `es_certificacion_externa` (retirado en la ronda 4), `normas_internas_17` confirmado FUERA de alcance (es una tabla base `NORMAS` aparte, huérfana, sección 1.7 de la spec), `PARCELA_CERTIFICACIONES` de presencia pura sin columna `estado` (a diferencia del boceto original), y las 8 columnas planas de `PADRON_SOCIOS` **NO se retiran** en esta migración (quedan congeladas como respaldo — el boceto original decía backfill-y-DROP, decisión revisada explícitamente en la ronda 4). `createSocio`/`updateSocio`/CSV ya reescritos contra las tablas nuevas. Pendiente, diferido a propósito: `lib/sociosSearch.js`/`page.jsx`/`SocioFormModal.jsx` siguen mostrando las 8 columnas viejas al usuario final (sección 8.1 de la spec) — tarea de rediseño de UI, no iniciada. Tests: tests/test_certificaciones_normalizadas.py (16 estáticos + 5 funcionales Live, auto-skip hasta aplicar la migración), tests/test_certificaciones_sociosactions_code_sites.mjs (8 estructurales), casos nuevos en tests/test_padron_csv.mjs.
- Pendiente: multi-producto/vertical — no iniciado. El hallazgo de seguridad sobre vw_parcelas_web (GRANTs de escritura completos a anon/authenticated sin uso en el código, ver Paso 1b) sigue sin resolver, hilo aparte.
